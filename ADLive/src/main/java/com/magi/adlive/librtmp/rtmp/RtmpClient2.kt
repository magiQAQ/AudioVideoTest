package com.magi.adlive.librtmp.rtmp
...
import com.magi.adlive.librtmp.amf.v0.AmfNumber
import com.magi.adlive.librtmp.amf.v0.AmfObject
...
import com.magi.adlive.librtmp.utils.AuthUtil
import com.magi.adlive.librtmp.utils.ConnectCheckerRtmp
import com.magi.adlive.librtmp.utils.CreateSSLSocket
...
class RtmpClient2(private val connectCheckerRtmp: ConnectCheckerRtmp) {
    ...
    fun connect(url: String?, isRetry: Boolean = false) {
        ...
        isStreaming = true
        readThread = HandlerThread("RtmpReader")
        writeThread = HandlerThread("RtmpWriter")
        readThread!!.start()
        writeThread!!.start()
        readHandler = Handler(readThread!!.looper)
        writeHandler = Handler(writeHandler!!.looper)
        readHandler!!.post {
            try {
                ...
                if (writer == null || reader == null) throw IOException("connect failed")
                commandsManager.sendConnect("", writer!!)
                while (!Thread.interrupted() && !publishPermitted) {
                    handleMessage()
                }
                Log.i(TAG, "It can push stream")
    ...
    private fun establishConnection(): Boolean {
        ...
        if (!tlsEnabled) {
            socket = Socket()
            val socketAddress = InetSocketAddress(commandsManager.host, commandsManager.port)
    ...
    private fun handleMessage() {
        ...
        when (message.getType()) {
            ...
            MessageType.SET_PEER_BANDWIDTH -> {
                ...
                writeHandler?.post { commandsManager.sendWindowAcknowledgementSize(writer) }
            }
            MessageType.ABORT -> {
            ...
            MessageType.USER_CONTROL -> {
                ...
                when (val type = userControl.type) {
                    Type.PING_REQUEST -> {
                        writeHandler?.post { commandsManager.sendPong(userControl.event, writer) }
                    }
                    else -> {
                        ...
                        when (commandName) {
                            "connect" -> {
                                if (commandsManager.onAuth) {
                                    mainHandler.post { connectCheckerRtmp.onAuthSuccessRtmp() }
                                    commandsManager.onAuth = false
                                }
                                writeHandler?.post { commandsManager.createStream(writer) }
                            }
                            "createStream" -> {
                                try {
                                    commandsManager.streamId = (command.data[3] as AmfNumber).value.toInt()
                                    writeHandler?.post { commandsManager.sendPublish(writer) }
                            ...
                            when (commandName) {
                                "connect" -> {
                                    if (description.contains("reason=authfail") || description.contains("reason=nosuchuser")) {
                                        mainHandler.post { connectCheckerRtmp.onAuthErrorRtmp() }
                                    ...
                                    mainHandler.post { connectCheckerRtmp.onConnectionFailedRtmp(description) }
                                }
                            }
                        } catch (e: ClassCastException) {
                        ...
                        try {
                            when (val code = ((command.data[3] as AmfObject).getProperty("code") as AmfString).value) {
                                "NetStream.Publish.Start" -> {
                                    ...
                                    mainHandler.post { connectCheckerRtmp.onConnectionSuccessRtmp() }
                                ...
                                "NetConnection.Connect.Rejected", "NetStream.Publish.BadName" -> {
                                    mainHandler.post { connectCheckerRtmp.onConnectionFailedRtmp("onStatus: $code") }
                                }
                                else -> {
    ...
    fun reConnect(delay: Long, backupUrl: String? = null) {
        ...
        runnable?.let { mainHandler.postDelayed(it, delay) }
    }
    ...
    private fun disconnect(clear: Boolean) {
        ...
        readThread?.looper?.thread?.interrupt()
        readThread?.looper?.quit()
        readThread?.quit()
        readThread = null
        try {
            readThread?.join(100)
